...
use either::Either;
use tryhcs_shared::{api_params::ErrorMessage, institution_params::{AuthenticatedUser, DepartmentDto, DepartmentId, LoginReq, LoginResponse, StaffDto, StaffId, VerifyOTP}};

#[async_trait::async_trait]
pub trait HcsEndpoints: Send + Sync  {
        async fn login(&self, login_req: &LoginReq) -> eyre::Result<LoginResponse, ErrorMessage>;
    async fn verify_otp(&self, verify_otp: &VerifyOTP) -> eyre::Result<AuthenticatedUser, ErrorMessage>;
