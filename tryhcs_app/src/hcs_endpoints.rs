...
use tryhcs_shared::{
    ...
    institution_params::{
        AuthenticatedUser, AuthorizedUser, CreateDepartment, CreateInstitution,
        DepartmentAndStaffDto, DepartmentDto, DepartmentId, DepartmentShadowId, InitiatedOtp,
        InstitutionDto, LoginReq, LoginResponse, NewStaff, StaffDto, StaffId, StaffShadowId,
        VerifyOTP,
    },
};
...
pub trait HcsEndpoints: Send + Sync {
    ...
    async fn delete_department(&self, id: &DepartmentShadowId) -> eyre::Result<(), ErrorMessage>;

    async fn add_staff(&self, req: &NewStaff) -> eyre::Result<StaffDto, ErrorMessage>;
    ...
    async fn delete_staff(&self, staff_id: &StaffShadowId) -> eyre::Result<(), ErrorMessage>;
