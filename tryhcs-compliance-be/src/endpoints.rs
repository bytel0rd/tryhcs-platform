...
use crate::{api, app::App, params::WorkspaceAdmin};


pub  fn compliance_router(app: Arc<App>) -> eyre::Result<Router<Arc<App>>> {
    let router = Router::new()
        .route("/compliance/v1/onboard/overview", get(get_compliance_status))
        .route("/compliance/v1/onboard/corporate", post(update_corporate_compliance))
        .route("/compliance/v1/onboard/finance", post(update_financial_compliance))
        .route("/compliance/v1/onboard/healthcare", post(update_healthcare_compliance))
        .route("/compliance/v1/onboard/submit", post(submit_compliance))
        .with_state(app)
        ;
    return Ok(router);
}
